...
> **Where things are.** This file is `skills/kit-qa/SKILL.md` inside the flutter-kit plugin; the plugin root is two folders above it. Before anything else set, in the shell, `PLUGIN` to that root (Claude Code: `${CLAUDE_PLUGIN_ROOT}`) and `KIT="$PLUGIN/kit/kit.sh"`; every `kit` command below runs as `bash "$KIT" …`, in the project folder. `$ARGUMENTS` means the words after `$kit-qa` in the user's message. Agents named in bold (flutter-tester, flutter-qa, …) are the Codex custom agents `$kit-codex-setup` installs into the project's `.codex/agents/`; spawn them by name, and where one is not installed do that part yourself.
...
at milestones (e.g. before `$kit-ship`, which runs it, after a refactor, or when
something feels off).
...
- If the user defers the fixes, record the defects durably: add a step
  ...
  `$PLUGIN/templates/plan/step.yaml.template`; title
  "Fix defects from $kit-qa <scope>", no dependencies, the defect list verbatim in
  its Description), run `kit validate`, so `$kit-next` and `$kit-step` surface them.
