...
> **Where things are.** This file is `skills/kit-step/SKILL.md` inside the flutter-kit plugin; the plugin root is two folders above it. Before anything else set, in the shell, `PLUGIN` to that root (Claude Code: `${CLAUDE_PLUGIN_ROOT}`) and `KIT="$PLUGIN/kit/kit.sh"`; every `kit` command below runs as `bash "$KIT" …`, in the project folder. `$ARGUMENTS` means the words after `$kit-step` in the user's message. Agents named in bold (flutter-tester, flutter-qa, …) are the Codex custom agents `$kit-codex-setup` installs into the project's `.codex/agents/`; spawn them by name, and where one is not installed do that part yourself.


# $kit-step — Implement the next plan step

The plan lives in `plan/` (schema: `$PLUGIN/schema/README.md`).
...
```bash
bash "$KIT" next --step     # the id to work
bash "$KIT" show <id>       # the step, as markdown
bash "$KIT" blocks <id>     # what it waits on
```
...
5. **Static gates.** Run, then record each:
   ...
   - `flutter analyze` — clean → `bash "$KIT" gate <id> analyze passed`
   - `flutter test` — the workflow's **single full-suite run**; inner agents
     run only affected files → `bash "$KIT" gate <id> tests passed --note "<count> tests, 0 skips"`
...
6. **Runtime verification (gating).** Run the step's integration tests
   ...
   defects. Record it: `bash "$KIT" gate <id> qa passed --note "<n>/<m> integration, 0 runtime errors"`.
...
8. **Human items.** Anything the step produced that needs the user's hands,
   ...
   ```bash
   bash "$KIT" item new --id <step>-<slug> \
...
9. **Mark complete.**
   ...
   refusal lists them), and point at `$kit-next`. Never `--force`.
