...
> **Where things are.** This file is `skills/kit-init-app/SKILL.md` inside the flutter-kit plugin; the plugin root is two folders above it. Before anything else set, in the shell, `PLUGIN` to that root (Claude Code: `${CLAUDE_PLUGIN_ROOT}`) and `KIT="$PLUGIN/kit/kit.sh"`; every `kit` command below runs as `bash "$KIT" …`, in the project folder. `$ARGUMENTS` means the words after `$kit-init-app` in the user's message. Agents named in bold (flutter-tester, flutter-qa, …) are the Codex custom agents `$kit-codex-setup` installs into the project's `.codex/agents/`; spawn them by name, and where one is not installed do that part yourself.


# $kit-init-app — Project initialization
...
- **Completeness is the goal.** The deliverable is a spec so thorough that after
  ...
  `HUMAN_SETUP.md`. Use `$PLUGIN/reference/requirements-checklist.md`
...
- **Kit files vs. project files.** Everything under `$PLUGIN/` is
...
Walk **`$PLUGIN/reference/requirements-checklist.md`** category by
...
```bash
cp "$PLUGIN/templates/PRODUCT_SPEC.md.template" PRODUCT_SPEC.md
cp "$PLUGIN/templates/CLAUDE.md.template"       CLAUDE.md
cp "$PLUGIN/templates/HUMAN_SETUP.md.template"   HUMAN_SETUP.md
mkdir -p plan/steps plan/items
cp "$PLUGIN/templates/plan/kit.yaml.template"    plan/kit.yaml
```
Then, in order:
...
3. **`plan/kit.yaml`** — replace every `<PLACEHOLDER>`: project name, the
   ...
   are about to write). Schema: `$PLUGIN/schema/README.md`.
4. **`plan/steps/*.yaml`** — **derive the step ladder from `PRODUCT_SPEC.md`**:
   ...
   file per step, from `$PLUGIN/templates/plan/step.yaml.template`;
   ...
   **staging** project, never production — immediately before release prep,
   ...
   ```bash
   bash "$KIT" validate      # must be clean
   bash "$KIT" render plan   # writes PROJECT_PLAN.md
   ```
5. **`HUMAN_SETUP.md` → items.** Remove the checklist entries that don't
   ...
   **item** so it gates the step it belongs to — one file each under
   `plan/items/`, from `$PLUGIN/templates/plan/item.yaml.template`
   ...
   delete `HUMAN_SETUP.md` — the board (`$kit-board`) is where the user reads
...
6. **Permissions** — write the project's Claude Code permissions so the build
   ...
   ```bash
   mkdir -p .claude && cp "$PLUGIN/templates/settings.json.template" .claude/settings.json
   ```
...
7. **Flutter rules** — copy the authoritative rule set into the project, because
   ...
   ```bash
   mkdir -p docs && cp "$PLUGIN/reference/flutter-rules.md" docs/flutter-rules.md
   ```
   This is the one kit file that gets pinned into the project. `$kit-kit-sync`
...
8. **`.gitignore`** — `flutter create` generated one in Stage 5. **Append** the
   ...
   `$PLUGIN/templates/gitignore.template`
8. **CI workflow** —
   ```bash
...
9. **Build journal** — create `docs/BUILD_NOTES.md` with the one-line header:
   `Per-project build journal — appended by $kit-step and $kit-qa; read at the start of every step.`
...
Each $kit-step builds a slice of the spec, then verifies the dev flavor on the iOS
simulator against the local Firebase emulators (optional demo fakes for edge
states) — checking every flow + dependent flows
for bugs, exceptions, and overflow (size-matrix widget tests). $kit-qa runs the
...
- Don't edit anything under `$PLUGIN/` — the plugin is read-only
