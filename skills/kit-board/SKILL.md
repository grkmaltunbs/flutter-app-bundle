...
> **Where things are.** This file is `skills/kit-board/SKILL.md` inside the flutter-kit plugin; the plugin root is two folders above it. Before anything else set, in the shell, `PLUGIN` to that root (Claude Code: `${CLAUDE_PLUGIN_ROOT}`) and `KIT="$PLUGIN/kit/kit.sh"`; every `kit` command below runs as `bash "$KIT" …`, in the project folder. `$ARGUMENTS` means the words after `$kit-board` in the user's message. Agents named in bold (flutter-tester, flutter-qa, …) are the Codex custom agents `$kit-codex-setup` installs into the project's `.codex/agents/`; spawn them by name, and where one is not installed do that part yourself.


# $kit-board — Pick up, regenerate, republish
...
1. **Pick up what was sent.** Read the live page with the Artifact tool
   ...
   ```bash
   bash "$KIT" inbox <scratch>/outbox.json --dry-run   # read it back to the user first
   bash "$KIT" inbox <scratch>/outbox.json
   ```
