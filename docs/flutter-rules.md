...
> **This document is the primary, authoritative rule set for Flutter and Dart
> work in this project.** When any other file in this bundle (agents, commands,
> skills, README) conflicts with a rule here, this document wins — *except* for
> the project's deliberately chosen stack.
>
...
> `very_good_analysis` is a stricter superset of `flutter_lints`, so it satisfies
> this document rather than conflicting with it.
>
> Everything else in this document — style, async, null safety, layout, theming,
> accessibility, documentation, layering, routing, serialization, and testing
> conventions (including `mocktail` for mocks and `package:checks` for
> assertions) — applies as written.
...
* **Logging:** Use the `log` function from `dart:developer` instead of `print`.
  See [Logging](#logging) for structured-logging guidance.
...
* **Mocks:** Prefer fakes or stubs over mocks. If mocks are absolutely
  ...
  code generation is common for state management (e.g., with `freezed`), avoid
  it for mocks — `mocktail` needs no `build_runner` step.
