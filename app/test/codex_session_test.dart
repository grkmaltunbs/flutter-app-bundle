...
void main() {
  ...
  test('Start on Codex: app-server, the handshake by itself, the thread as the session; the record says codex', () async {
    ...
    expect(fake.requests.map((r) => r.method), ['initialize', 'config/read', 'model/list', 'skills/list', 'thread/start', 'account/rateLimits/read']);
  ...
  test('model evidence follows the accepted turn, survives selection changes, and default resets explicitly', () async {
    final fake = FakeCodex();
    final s = codexSession(fake, dir: project.path, home: home.path);
    await s.start();
    await fake.requested('thread/start');
    await pumpEventQueue();
    expect(s.confirmedModel, isNull, reason: 'thread configuration alone is not a turn');
    expect(s.modelConfirmation, ModelConfirmation.unknown);
    s.send('first');
    await fake.requested('turn/start');
    await pumpEventQueue();
    expect(s.confirmedModel, 'gpt-6-astra');
    expect(s.modelConfirmation, ModelConfirmation.confirmed);
    fake.scriptTurn(text: 'First answer');
    await pumpEventQueue();
    final first = s.transcript.messages.last;
    expect(first.model, 'gpt-6-astra');
    expect(DeckMessage.fromMap(first.toMap()).model, first.model);
    await s.selectModel('gpt-5.5');
    expect(s.modelConfirmation, ModelConfirmation.pending);
    expect(s.confirmedModel, 'gpt-6-astra');
    expect(first.model, 'gpt-6-astra');
    s.send('second');
    await fake.requested('turn/start', times: 2);
    await pumpEventQueue();
    expect(s.confirmedModel, 'gpt-5.5');
    expect(s.modelConfirmation, ModelConfirmation.confirmed);
    await s.selectModel('default');
    fake.notify('model/rerouted', {'threadId': 'other-thread', 'turnId': fakeTurn, 'fromModel': 'gpt-5.5', 'toModel': 'wrong'});
    fake.notify('model/rerouted', {'threadId': fakeThread, 'turnId': 'old-turn', 'fromModel': 'gpt-5.5', 'toModel': 'wrong'});
    await pumpEventQueue();
    expect(s.confirmedModel, 'gpt-5.5');
    fake.notify('model/rerouted', {'threadId': fakeThread, 'turnId': fakeTurn, 'fromModel': 'gpt-5.5', 'toModel': 'gpt-6-astra'});
    await pumpEventQueue();
    expect(s.modelConfirmation, ModelConfirmation.pending, reason: 'a reroute of the old turn cannot acknowledge the new selection');
    expect(s.confirmedModel, 'gpt-6-astra');
    expect(first.model, 'gpt-6-astra');
    expect(s.transcript.messages.any((m) => m.text.contains('rerouted this turn')), isTrue);
    fake.scriptTurn(text: 'Second answer');
    await pumpEventQueue();
    s.send('default again');
    await fake.requested('turn/start', times: 3);
    await pumpEventQueue();
    final params = fake.lastParams('turn/start');
    expect(params['model'], 'gpt-6-astra');
    expect((params['collaborationMode'] as Map)['settings'], {'model': 'gpt-6-astra'});
    expect(s.modelConfirmation, ModelConfirmation.confirmed);
    expect(s.toRelay()['confirmedModel'], 'gpt-6-astra');
    expect(s.toRelay()['modelConfirmation'], 'confirmed');
    await expectLater(s.selectModel('invented-model'), throwsArgumentError);
    expect(s.modelChoice, isNull);
    await s.stop();
  });

  test('a rejected turn and unacknowledged settings never confirm the selected model', () async {
    final fake = FakeCodex(autoTurn: false);
    final s = codexSession(fake, dir: project.path, home: home.path);
    await s.start();
    await fake.requested('thread/start');
    await pumpEventQueue();
    ...
    s.send('try this');
    await fake.requested('turn/start');
    fake.notify('thread/settings/updated', {'threadId': fakeThread, 'threadSettings': {'model': 'gpt-5.5'}});
    await pumpEventQueue();
    expect(s.confirmedModel, isNull);
    expect(s.modelConfirmation, ModelConfirmation.pending);
    fake.emitJson({'id': fake.requestIds['turn/start'], 'error': {'message': 'model unavailable'}});
    await pumpEventQueue();
    expect(s.state, BridgeState.ready);
    expect(s.confirmedModel, isNull);
    expect(s.modelConfirmation, ModelConfirmation.pending);
  ...
  test('unchanged model confirms only on acceptance and labels the next response without a settings update', () async {
    final fake = FakeCodex(autoTurn: false);
    final s = codexSession(fake, dir: project.path, home: home.path);
    await s.start();
    await fake.requested('thread/start');
    await pumpEventQueue();
    s.send('unchanged model');
    await fake.requested('turn/start');
    expect(s.modelConfirmation, ModelConfirmation.pending);
    expect(s.confirmedModel, isNull);
    await s.selectModel('gpt-6-astra');
    fake.emitJson({'id': fake.requestIds['turn/start'], 'result': {'turn': {'id': fakeTurn}}});
    await pumpEventQueue();
    expect(s.confirmedModel, 'gpt-6-astra');
    expect(s.modelConfirmation, ModelConfirmation.pending, reason: 'selection changed midturn even though the effective model is identical');
    fake.scriptTurn(text: 'First answer');
    await pumpEventQueue();
    final first = s.transcript.messages.last;
    expect(first.model, 'gpt-6-astra');
    s.send('explicitly selected same model');
    await fake.requested('turn/start', times: 2);
    fake.emitJson({'id': fake.requestIds['turn/start'], 'result': {'turn': {'id': fakeTurn}}});
    await pumpEventQueue();
    expect(s.modelConfirmation, ModelConfirmation.confirmed);
    fake.scriptTurn(text: 'Second answer');
    await pumpEventQueue();
    expect(s.transcript.messages.last.model, 'gpt-6-astra');
    expect(first.model, 'gpt-6-astra');
    await s.stop();
  });
  ...
  testWidgets('the Deck on Codex: the ENGINE notch, Codex\'s dials, the facts line, the browser pill; no overflow at three scales', (tester) async {
    for (final scale in const [1.0, 2.0, 3.12]) {
      ...
      await tester.ensureVisible(find.byType(Slider).at(2));
