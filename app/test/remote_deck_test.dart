...
void main() {
  ...
  testWidgets('the option pills on the phone are commands; the session document is what they show', (tester) async {
    ...
    expect(find.text('BROWSER · OFF'), findsOneWidget, reason: 'a Codex session with no browser server listed');
    await project.set({'session': {'mode': 'bridge', 'state': 'ready', 'chromeStatus': 'ready'}}, SetOptions(merge: true));
    await _settle(tester);
    expect(find.text('BROWSER · READY'), findsNothing, reason: 'running: the controls fold');
    await tester.tap(find.byTooltip('Show session controls'));
    await _settle(tester);
    expect(find.text('BROWSER · READY'), findsOneWidget, reason: 'live, the Browser use runtime up');
    await project.set({'session': {'mode': 'idle', 'state': 'stopped', 'chromeStatus': FieldValue.delete()}}, SetOptions(merge: true));
    await _settle(tester);
    ...
    await project.set({'session': {'engine': 'claude', 'models': FieldValue.delete(), 'cliVersion': FieldValue.delete(), 'rules': FieldValue.delete(), 'chromeStatus': FieldValue.delete()}}, SetOptions(merge: true));
    await _settle(tester);
    expect(find.text('ENGINE · CLAUDE'), findsOneWidget);
